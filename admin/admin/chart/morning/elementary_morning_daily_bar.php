...
$late = array();
for( $m = 1; $m <= 31; $m++ ) {
  $sql = "SELECT DISTINCT student_id FROM monitoring_morning WHERE Day(date) = '$m' AND status_morning = 1 AND student = 'Elementary' $and";
  $oquery = $conn->query($sql);
  array_push($ontime, $oquery->num_rows);

  $sql = "SELECT DISTINCT student_id FROM monitoring_morning WHERE Day(date) = '$m' AND status_morning = 0 AND student = 'Elementary' $and";
...
$('#select_year').change(function(){
  window.location.href = 'elementary_morning_daily.php?month='+$(this).val();
});
});
</script>