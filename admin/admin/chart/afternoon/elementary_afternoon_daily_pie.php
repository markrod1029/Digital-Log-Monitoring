...
<?php
 ...
 //  late 
 $sql = "SELECT DISTINCT student_id FROM monitoring_afternoon WHERE date = '$today' AND status_afternoon = 0  AND student = 'Elementary'";
 $query = $conn->query($sql);
 $late = $query->num_rows;
 
 $sql = "SELECT DISTINCT student_id FROM monitoring_afternoon WHERE date = '$today' AND status_afternoon = 1  AND student = 'Elementary'";
 $query = $conn->query($sql);
...
var newVSReturningVisitorsOptions = {
	...
	title: {
		text: "Total Percentage of Late and Ontime Today <br> <?php echo  $todays ?>"
	},
...
<p class="h4 mx-auto text-center">The Data shown in the figures shows that <b><?php echo number_format((float)$ontime1, 2, '.', '')?>%</b> of the 
	Elementary Students are <b>Ontime</b>, and <b><?php echo  number_format((float)$late1, 2, '.', '') ?>%</b>  are Late.
	 This means that most of the time the Elementary Students are  <b><?php echo  $student?></b>  </p>
  <br>