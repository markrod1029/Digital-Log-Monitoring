...
$late = array();
for( $m = 1; $m <= 31; $m++ ) {
  $sql = "SELECT DISTINCT student_id FROM monitoring_afternoon WHERE Day(date) = '$m' AND status_afternoon = 1 AND student = 'Senior' $and";
  $oquery = $conn->query($sql);
  array_push($ontime, $oquery->num_rows);

  $sql = "SELECT DISTINCT student_id FROM monitoring_afternoon WHERE Day(date) = '$m' AND status_afternoon = 0 AND student = 'Senior' $and";
...
$('#select_year').change(function(){
  window.location.href = 'senior_afternoon_daily.php?month='+$(this).val();
});
});
</script>