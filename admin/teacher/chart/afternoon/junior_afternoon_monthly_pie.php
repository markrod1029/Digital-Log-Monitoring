...
<?php
 ...
 //  late 
  $sql = "SELECT DISTINCT student_id FROM monitoring_afternoon WHERE year = '$year' AND month = '$month' AND status_afternoon = 0  AND student = 'Junior'";
 $query = $conn->query($sql);
 $late = $query->num_rows;
 // ontime
 
 $sql = "SELECT DISTINCT student_id FROM monitoring_afternoon WHERE   year = '$year' AND month = '$month'  AND status_afternoon = 1 AND student = 'Junior'";
 $query = $conn->query($sql);
...
<p class="h4 mx-auto text-center">The Data shown in the figures shows that <b><?php echo number_format((float)$ontime1, 2, '.', '')?>%</b> of the 
	Junior Students are <b>Ontime</b>, and <b><?php echo  number_format((float)$late1, 2, '.', '') ?>%</b>  are Late.
	 This means that most of the time the Junior Students are  <b><?php echo  $student?></b>  </p>
  <br>