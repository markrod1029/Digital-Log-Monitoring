...
for( $m =1; $m <=7; $m++ ) {
  ...
  $sql = "SELECT * FROM attendance WHERE DAY(date) = '$m' AND status = 1 $and";
  $oquery = $conn->query($sql);

  $sql = "SELECT * FROM attendance WHERE DAY(date) = '$m' AND status = 0 $and";
  ...
  $month =  date('W', mktime(0, 0, 0, 12, $m, $y));
...
$('#select_year').change(function(){
  window.location.href = 'ontime.php?week='+$(this).val();
});
});
</script>