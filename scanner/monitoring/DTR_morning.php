        ...
        if($query->num_rows <= 0){
			...
			if($query->num_rows > 0 ){
					...
					$row = $query->fetch_assoc(); 
							...
							$sql = "UPDATE monitoring SET student = '$position', time_out = '$currentTimeFormatted', num_hr = '$int',  status = '1'  WHERE student_id = '$id' AND date = '$date_now'  AND status = '0' ";
							$query = $conn->query($sql);
				...
				if( $morning == 'PM')
				{
					...
					<h4>You cannot Time In Already Afternoon</div>
				";
				...
				$query = $conn->query($sql);
				if($query->num_rows > 0){
					echo "<div class='alert alert-warning' role='alert' style='font-size:22px'><h4><i class='fa fa-clock'></i>  you already Time In today</h4></div>
					...
					$sql = "INSERT INTO monitoring (student_qrcode, student, student_id, schedule_id,  date, time_in, status_monitoring, day, month, year, time ) VALUES ('$student', '$schedule_id', '$id', '$sched', '$date_now', '$currentTimeFormatted', '$logstatus', '$day', $month, '$year', 'Morning')";
