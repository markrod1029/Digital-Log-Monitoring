...
include('session.php');
$tuserr_id = $_POST['user_id'];
$my_message = $_POST['my_message'];


$query = mysqli_query($conn,"select * from user_id where user_id = '$teacher_id'")or die(mysqli_error());
$row = mysqli_fetch_array($query);
$name = $row['firstname']." ".$row['lastname'];

$query1 = mysqli_query($conn,"select * from teacher where user_id = '$session_id'")or die(mysqli_error());
