	...
	if(isset($_POST['login'])){
        ...
        if ($query->num_rows >= 1) {
			...
			if(password_verify($password, $row['password'])){
				...
				$status = "Active now";
				$id = $row['id'];
			$sql = mysqli_query($conn, "UPDATE admin_table SET chat_status = '{$status}' WHERE id='$id'");
			}
			...
			if(password_verify($password, $row1['doctor_password'])){
				...
				$status = "Active now";
				$id = $row1['id'];
			$sql = mysqli_query($conn, "UPDATE doctor_table SET chat_status = '{$status}' WHERE id='$id'");
			}
				...
				$status = "Active now";
				$id = $row2['id'];
				$sql = mysqli_query($conn, "UPDATE patient_table SET chat_status = '{$status}' WHERE id='$id'");
				}
