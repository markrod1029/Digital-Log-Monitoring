...
$doctor_id = $_SESSION['admin_id'];
    ...
    while($row1 = mysqli_fetch_assoc($query1)){
        $sql2 = "SELECT * FROM messages WHERE (incoming_msg_id = {$row1['doctor_id']}
                OR outgoing_msg_id = {$row1['doctor_id']}) AND (outgoing_msg_id = {$outgoing_id} 
                OR incoming_msg_id = {$outgoing_id}) ORDER BY msg_id DESC ";
        $query2 = mysqli_query($conn, $sql2);
        $row2 = mysqli_fetch_assoc($query2);
        (mysqli_num_rows($query2) > 0) ? $result = $row2['msg'] : $result ="No message available";
        ...
        $output .= '<a href="chat.php?patient_id='. $row1['doctor_id'] .' ">
                    <div class="content" '. $highlight .' >
                    <img src="../images/'. $row1['doctor_profile_image'] .'" alt="">
                    <div class="details">
                        <span '. $highlight .' >'. $row1['doctor_name']. '</span>
                        <p>'. $you . $msg .'</p>
                    </div>
                    </div>
                    <div class="status-dot '. $offline .'"><i class="fas fa-circle"></i></div>
